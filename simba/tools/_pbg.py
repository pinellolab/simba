...
def gen_graph(
    ...
    list_KA: `list`, optional (default: None)
        A list of anndata objects that store relation between Kmers and Annotations
    ...
    if (
        sum(
            list(
                map(
                    lambda x: x is None,
                    [list_CP, list_PM, list_PK, list_CG, list_CC, list_adata],
                )
            )
        )
        == 6
    ...
    settings.pbg_params["entity_path"] = os.path.join(filepath, "input/entity")
    ...
    def _get_df_edges(
        ...
        df_edges_x["source"] = df_source.loc[
            adata.obs_names[adj_mat.nonzero()[0]], "alias"
        ].values
        df_edges_x["relation"] = relation_id
        df_edges_x["destination"] = df_dest.loc[
            adata.var_names[adj_mat.nonzero()[1]], "alias"
        ].values
        if include_weight:
    ...
    if list_adata is not None:
        ...
        if get_marker_significance and list_adata:
            raise NotImplementedError(
                "Marker gene significance is not yet implemented for graph generation from AnnData list."
            )
        for ctr_rel, adata_ori in enumerate(list_adata):
            ...
            if len(set(var_names).intersection(id_ent)) == 0:
                ...
                entity_alias_var = pd.DataFrame(
                    index=var_names,
                    columns=["alias"],
                    data=[f"{prefix_i}.{x}" for x in range(len(var_names))],
                )
                settings.pbg_params["entities"][prefix_i] = {"num_partitions": 1}
                dict_ent_type[prefix_i] = var_names
                entity_alias = pd.concat(
                    [entity_alias, entity_alias_var], ignore_index=False
                )
                var_type = prefix_i
                ctr_ent += 1
            else:
            ...
            if layer is not None:
                if layer in adata_ori.layers.keys():
                    ...
                    print(
                        f"`{layer}` does not exist in adata {ctr_rel} "
            ...
            adata_ori.obs.loc[obs_names, "pbg_id"] = entity_alias.loc[
                obs_names, "alias"
            ].copy()
            adata_ori.var.loc[var_names, "pbg_id"] = entity_alias.loc[
                var_names, "alias"
            ].copy()
        ...
        if list_KA is not None:
            for adata_ori in list_KA:
                adata = adata_ori.copy()
                ids_kmers = ids_kmers.union(adata.obs.index)
                ids_annots = ids_annots.union(adata.var.index)
            if get_marker_significance:
                n_nannots = int(len(ids_annots) * fold_null_nodes)
                ids_nannots = pd.Index([f"n{prefix_K}.{x}" for x in range(n_nannots)])
        ...
        for k in dict_cells.keys():
            ...
            entity_alias = pd.concat(
                [entity_alias, dict_df_cells[k]], ignore_index=False
            )
        if len(ids_genes) > 0:
            ...
            if get_marker_significance and (len(ids_ngenes) > 0):
                df_ngenes = pd.DataFrame(
                    index=ids_ngenes, columns=["alias"], data=ids_ngenes.tolist()
                )
                settings.pbg_params["entities"][f"n{prefix_G}"] = {"num_partitions": 1}
                entity_alias = pd.concat([entity_alias, df_ngenes], ignore_index=False)
        if len(ids_peaks) > 0:
            ...
            if get_marker_significance and (len(ids_npeaks) > 0):
                df_npeaks = pd.DataFrame(
                    index=ids_npeaks, columns=["alias"], data=ids_npeaks.tolist()
                )
                settings.pbg_params["entities"][f"n{prefix_P}"] = {"num_partitions": 1}
                entity_alias = pd.concat([entity_alias, df_npeaks], ignore_index=False)
        if len(ids_kmers) > 0:
            ...
            if get_marker_significance and (len(ids_nkmers) > 0):
                df_nkmers = pd.DataFrame(
                    index=ids_nkmers, columns=["alias"], data=ids_nkmers.tolist()
                )
                settings.pbg_params["entities"][f"n{prefix_K}"] = {"num_partitions": 1}
                entity_alias = pd.concat([entity_alias, df_nkmers], ignore_index=False)
        if len(ids_annots) > 0:
            df_annots = pd.DataFrame(
                index=ids_annots,
                columns=["alias"],
                data=[f"{prefix_M}.{x}" for x in range(len(ids_annots))],
            )
            settings.pbg_params["entities"][prefix_M] = {"num_partitions": 1}
            entity_alias = pd.concat([entity_alias, df_annots], ignore_index=False)
            if get_marker_significance and (len(ids_nannots) > 0):
                df_nannots = pd.DataFrame(
                    index=ids_nannots, columns=["alias"], data=ids_nannots.tolist()
                )
                settings.pbg_params["entities"][f"n{prefix_M}"] = {"num_partitions": 1}
                entity_alias = pd.concat([entity_alias, df_nannots], ignore_index=False)
        if len(ids_motifs) > 0:
            ...
            if get_marker_significance and (len(ids_nmotifs) > 0):
                df_nmotifs = pd.DataFrame(
                    index=ids_nmotifs, columns=["alias"], data=ids_nmotifs.tolist()
                )
                settings.pbg_params["entities"][f"n{prefix_M}"] = {"num_partitions": 1}
                entity_alias = pd.concat([entity_alias, df_nmotifs], ignore_index=False)
        # generate edges
        dict_graph_stats = dict()
        if add_edge_weights:
            col_names = ["source", "relation", "destination", "weight"]
            ...
            for i, adata_ori in enumerate(list_CP):
                ...
                if layer is not None:
                    if layer in adata.layers.keys():
                        ...
                        print(
                            f"`{layer}` does not exist in anndata {i} "
                ...
                if get_marker_significance:
                    ...
                    null_matrix = _randomize_matrix(
                        arr_simba, n_npeaks, method="degPreserving"
                    )
                    null_adata = ad.AnnData(
                        obs=adata.obs, var=df_npeaks, layers={"disc": null_matrix}
                    )
                _row, _col = arr_simba.nonzero()
                df_edges_x = pd.DataFrame(columns=col_names)
                df_edges_x["source"] = df_cells.loc[
                    adata.obs_names[_row], "alias"
                ].values
                df_edges_x["relation"] = f"r{id_r}"
                df_edges_x["destination"] = df_peaks.loc[
                    adata.var_names[_col], "alias"
                ].values
                if add_edge_weights:
                    df_edges_x["weight"] = arr_simba[_row, _col].A.flatten()
                settings.pbg_params["relations"].append(
                ...
                adata_ori.obs["pbg_id"] = ""
                adata_ori.var["pbg_id"] = ""
                adata_ori.obs.loc[adata.obs_names, "pbg_id"] = df_cells.loc[
                    adata.obs_names, "alias"
                ].copy()
                adata_ori.var.loc[adata.var_names, "pbg_id"] = df_peaks.loc[
                ...
                if get_marker_significance:
                    ...
                    df_edges_x["destination"] = df_cells.loc[
                        null_adata.obs_names[_row], "alias"
                    ].values
                    df_edges_x["relation"] = f"r{id_r}"
                    df_edges_x["source"] = df_npeaks.loc[
                        null_adata.var_names[_col], "alias"
                    ].values
                    df_edges_x["weight"] = null_matrix.transpose()[
                        _col, _row
                    ].A.flatten()
                    settings.pbg_params["relations"].append(
                        {
        ...
        if list_PM is not None:
            for i, adata_ori in enumerate(list_PM):
                ...
                if get_marker_significance:
                    n_nmotifs = int(len(ids_motifs) * fold_null_nodes)
                    null_matrix = _randomize_matrix(
                        arr_simba, n_nmotifs, method="degPreserving"
                    )
                    null_adata = ad.AnnData(
                        obs=adata.obs, var=df_nmotifs, layers={"disc": null_matrix}
                    )
                _row, _col = arr_simba.nonzero()
                df_edges_x = pd.DataFrame(columns=col_names)
                ...
                adata_ori.obs["pbg_id"] = ""
                adata_ori.var["pbg_id"] = ""
                adata_ori.obs.loc[adata.obs_names, "pbg_id"] = df_peaks.loc[
                    adata.obs_names, "alias"
                ].copy()
                adata_ori.var.loc[adata.var_names, "pbg_id"] = df_motifs.loc[
                    adata.var_names, "alias"
                ].copy()
                if get_marker_significance:
                    _col, _row = null_matrix.transpose().nonzero()
        ...
        if list_PK is not None:
            for i, adata_ori in enumerate(list_PK):
                ...
                if get_marker_significance:
                    n_nkmers = int(len(ids_kmers) * fold_null_nodes)
                    null_matrix = _randomize_matrix(
                        arr_simba, n_nkmers, method="degPreserving"
                    )
                    null_adata = ad.AnnData(
                        obs=adata.obs, var=df_nkmers, layers={"disc": null_matrix}
                    )
                _row, _col = arr_simba.nonzero()
                df_edges_x = pd.DataFrame(columns=col_names)
                df_edges_x["source"] = df_peaks.loc[
                ...
                adata_ori.var.loc[adata.var_names, "pbg_id"] = df_kmers.loc[
                    adata.var_names, "alias"
                ].copy()
                if get_marker_significance:
                    _col, _row = null_matrix.transpose().nonzero()
        ...
        if list_KA is not None:
            for i, adata_ori in enumerate(list_PK):
                ...
                if get_marker_significance:
                    n_nannots = int(len(ids_annots) * fold_null_nodes)
                    null_matrix = _randomize_matrix(
                        arr_simba, n_nannots, method="degPreserving"
                    )
                    null_adata = ad.AnnData(
                        obs=adata.obs, var=df_nannots, layers={"disc": null_matrix}
                    )
                _row, _col = arr_simba.nonzero()
                df_edges_x = pd.DataFrame(columns=col_names)
                df_edges_x["source"] = df_kmers.loc[
                    adata.obs_names[_row], "alias"
                ].values
                df_edges_x["relation"] = f"r{id_r}"
                df_edges_x["destination"] = df_annots.loc[
                    adata.var_names[_col], "alias"
                ].values
                if add_edge_weights:
                    df_edges_x["weight"] = arr_simba[_row, _col].A.flatten()
                ...
                adata_ori.obs["pbg_id"] = ""
                adata_ori.var["pbg_id"] = ""
                adata_ori.obs.loc[adata.obs_names, "pbg_id"] = df_kmers.loc[
                    adata.obs_names, "alias"
                ].copy()
                adata_ori.var.loc[adata.var_names, "pbg_id"] = df_annots.loc[
                    adata.var_names, "alias"
                ].copy()
                if get_marker_significance:
                    _col, _row = null_matrix.transpose().nonzero()
        ...
        if list_CG is not None:
            for i, adata_ori in enumerate(list_CG):
                ...
                if get_marker_significance:
                    n_ngenes = int(len(ids_genes) * fold_null_nodes)
                    null_exp_matrix = _randomize_matrix(
                        arr_simba, n_ngenes, method="degPreserving"
                    )
                    null_adata = ad.AnnData(
                        obs=adata.obs, var=df_ngenes, layers={"disc": null_exp_matrix}
                    )
                if add_edge_weights:
                    _row, _col = arr_simba.nonzero()
                    df_edges_x = pd.DataFrame(columns=col_names)
                    ...
                    if get_marker_significance:
                        ...
                        df_edges_x["weight"] = null_exp_matrix.transpose()[
                            _col, _row
                        ].A.flatten()
                        settings.pbg_params["relations"].append(
                            {
                    ...
                    for i_lvl, lvl in enumerate(expr_level):
                        ...
                        if get_marker_significance:
                            ...
                            df_edges_v = _get_df_edges(
                                (null_exp_matrix == lvl).astype(int).T,
                                df_ngenes,
                                df_cells,
                                null_adata.transpose(),
                                f"r{id_r}",
                                include_weight=True,
                                weight_scale=lvl,
                            )
                            ...
                            df_edges = pd.concat(
                                [df_edges, df_edges_v], ignore_index=True
                            )
                            settings.pbg_params["relations"].append(
                                {
                                    "name": f"r{id_r}",
                ...
                adata_ori.var.loc[adata.var_names, "pbg_id"] = df_genes.loc[
                    adata.var_names, "alias"
                ].copy()
        ...
        if list_CC is not None:
            for i, adata in enumerate(list_CC):
                ...
                id_r += 1
                df_edges = pd.concat([df_edges, df_edges_x], ignore_index=True)
                adata.obs["pbg_id"] = df_cells_obs.loc[adata.obs_names, "alias"].copy()
                adata.var["pbg_id"] = df_cells_var.loc[adata.var_names, "alias"].copy()
    ...
    df_edges.to_csv(
        os.path.join(filepath, "pbg_graph.txt"), header=False, index=False, sep="\t"
    )
    entity_alias.to_csv(
        os.path.join(filepath, "entity_alias.txt"), header=True, index=True, sep="\t"
    )
    with open(os.path.join(filepath, "graph_stats.json"), "w") as fp:
        json.dump(
            dict_graph_stats, fp, sort_keys=True, indent=4, separators=(",", ": ")
        )
    print("Finished.")
    ...
    if get_marker_significance:
        ...
        settings.pbg_params["relations"] = settings.graph_stats[dirname_orig][
            "relations"
        ]
    if copy:
        return df_edges
...
def pbg_train(
    ...
    if pbg_params is None:
        ...
        assert isinstance(pbg_params, dict), "`pbg_params` must be dict"
    ...
    if get_marker_significance:
        ...
        n_edges = settings.graph_stats[os.path.basename(filepath)]["n_edges"]
        filepath += "_with_sig"
        pbg_params["checkpoint_path"] = os.path.join(filepath, output)
        ...
        pbg_params["relations"] = settings.graph_stats[dirname + "_with_sig"][
            "relations"
        ]
        auto_wd = False
        pbg_params["wd"] = (
            settings.pbg_params["wd"]
            * n_edges
            / settings.graph_stats[os.path.basename(filepath)]["n_edges"]
        )
        settings.pbg_params["wd"] = pbg_params["wd"]

    if auto_wd:
        # empirical numbers from simulation experiments
        if settings.graph_stats[os.path.basename(filepath)]["n_edges"] < 5e7:
